...
def audit_position(claims:list[dict[str,Any]],position:dict[str,Any],report:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    if source=="USER_DECLARED":
        projection=(report or {}).get("position_guidance") or {};invalid=projection.get("original_invalidation") or {}
        if invalid.get("stop")!=position.get("original_stop") or invalid.get("timeframe")!=position.get("original_timeframe") or invalid.get("thesis")!=position.get("original_thesis") or invalid.get("fact_ref")!="POSITION_ORIGINAL_STOP":codes.append("UNSUPPORTED_POSITION")
