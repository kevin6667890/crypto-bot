...
def run_discovery_candidate_backtest(candles: list[dict[str, Any]], instrument: str, timeframe: str,
                                     ...
                                     end_ts: int, execution: DiscoveryExecutionConfig | None = None) -> dict[str, Any]:
    ...
    def provider(candle: dict[str, Any], index: int) -> dict[str, Any]:
        ...
        return {"action": action, "atr": feature.get("atr"), "score": 0.0,
                "signal_ts": timestamp, "signal_id": f"discovery:{config_hash[:16]}:{timestamp}",
                "strategy_version": TEMPLATE_VERSION[template], "config_hash": config_hash,
    ...
    return run_execution_backtest(candles, instrument, timeframe, parameters, start_ts, end_ts, signal_provider=provider)
