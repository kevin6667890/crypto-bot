...
def normalize_template_parameters(template: str, parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    p={"fast_period":int(parameters.get("fast_period",20)),"slow_period":int(parameters.get("slow_period",200)),"fast_ma_type":str(parameters.get("fast_ma_type","EMA")),"atr_period":int(parameters.get("atr_period",14)),"volume_enabled":bool(parameters.get("volume_enabled",False))}
    ...
    if p["volume_enabled"]:
        ratio=round(float(parameters.get("minimum_volume_ratio",1.0)),2)
    ...
    if template == "TREND_PULLBACK":
        distance=round(float(parameters.get("maximum_distance",.004)),4)
    ...
    if template == "MEAN_REVERSION":
        low=int(parameters.get("rsi_lower",35)); high=int(parameters.get("rsi_upper",65))
...
def build_parameter_identity(template: str, parameters: dict[str, Any]) -> str:
    ...
    return canonical_json_hash({"parameter_identity_version":DISCOVERY_PARAMETER_IDENTITY_VERSION,"template":template,"template_version":TEMPLATE_VERSION[template],"feature_version":"discovery-features-v1","parameters":normalized})

def build_candidate_identity(template: str, parameters: dict[str, Any], execution_hash: str) -> str:
    return canonical_json_hash({"candidate_identity_version":DISCOVERY_CANDIDATE_IDENTITY_VERSION,"parameter_hash":build_parameter_identity(template,parameters),"execution_hash":execution_hash,"template_version":TEMPLATE_VERSION[template],"feature_version":"discovery-features-v1","execution_engine_version":SHARED_EXECUTION_ENGINE_VERSION})
