...
class HealthService:
    ...
    def payload(self,details:bool=False)->dict[str,Any]:
        ...
        ai_health=self.paper.ai_health() if callable(getattr(self.paper,"ai_health",None)) else {"status":"unknown","queue_depth":0,"instruments":{}}
        result={"status":status,"version":"4.1.0","git_commit":self._commit(),"uptime_seconds":int(time.monotonic()-self.started),"database_status":db_status,"database_size_bytes":self.db_path.stat().st_size if self.db_path.exists() else 0,"database_integrity_last_checked":self.integrity_at,"database_integrity_status":self.integrity_status,"paper_scheduler_running":bool(getattr(self.paper,"scheduler_running",False)),"last_cycle_started_at":getattr(self.paper,"last_cycle_started_at",None),"last_cycle_completed_at":getattr(self.paper,"last_cycle_completed_at",None),"last_cycle_duration_ms":getattr(self.paper,"last_cycle_duration_ms",None),"next_cycle_at":getattr(self.paper,"next_cycle_at",None),"collector_freshness":freshness,"flow_health":self.paper.flow_health() if callable(getattr(self.paper,"flow_health",None)) else {"paper_api_status":"unknown"},"last_okx_success":getattr(self.paper,"last_okx_success",None),"last_okx_error":getattr(self.paper,"last_okx_error",None),"active_job":active,"queued_jobs":queued,"deepseek_configured":bool(os.getenv("DEEPSEEK_API_KEY")),"last_ai_success":getattr(self.paper,"last_ai_success",None),"ai_health":ai_health,"disk_usage":{"used_bytes":disk.used,"total_bytes":disk.total,"percent":round(disk.used/disk.total*100,1)},"memory_usage":memory}
