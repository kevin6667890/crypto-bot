...
class ResearchRepository:
    ...
    def initialize(self) -> None:
        with self.connect() as connection:
            connection.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS canonical_dataset_metadata (metadata_key TEXT PRIMARY KEY, schema_version TEXT NOT NULL, partition_fingerprint_version TEXT NOT NULL, dataset_fingerprint_version TEXT NOT NULL, dataset_fingerprint TEXT NOT NULL, payload TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
    ...
    DISCOVERY_REJECTION_DESCRIPTIONS = {
        "INSUFFICIENT_FOLDS_WITH_TRADES": "Too few Development folds produced trades.", "INSUFFICIENT_TOTAL_TRADES": "The total Development trade count was below the minimum.",
        "INSUFFICIENT_MEDIAN_TRADES": "The median trades per Development fold was below the minimum.", "INSUFFICIENT_PROFITABLE_FOLDS": "Too few Development folds had a positive return.",
        "INSUFFICIENT_BENCHMARK_BEATING_FOLDS": "Too few Development folds beat the benchmark.", "NONPOSITIVE_MEDIAN_EXCESS_RETURN": "Median excess return relative to the benchmark was not positive.",
        "WORST_FOLD_RETURN_TOO_LOW": "The worst Development-fold return was below the allowed limit.", "WORST_EXCESS_RETURN_TOO_LOW": "The worst Development-fold excess return was below the allowed limit.",
    ...
    def automatic_research_diagnostics(self, cycle_id: int, *, page: int = 1, page_size: int = 25, reason: str | None = None, eligibility: str | None = None, search: str | None = None) -> dict[str, Any] | None:
        page, page_size = max(1, page), min(max(1, page_size), 100)
        with self.connect() as c:
            cycle = c.execute("SELECT discovery_run_id FROM automatic_research_cycles WHERE id=?", (cycle_id,)).fetchone()
            if not cycle: return None
            if not cycle["discovery_run_id"]: return {"cycle_id": cycle_id, "diagnostics_available": False}
            rows = [dict(row) for row in c.execute("SELECT id,candidate_number,template,program_version,candidate_identity,parameter_hash,parameters,direction,complexity,eligibility_status,development_score,elimination_reasons,aggregate_metrics FROM strategy_discovery_candidates WHERE discovery_run_id=? ORDER BY candidate_number", (cycle["discovery_run_id"],))]
        if not rows or not any(row.get("eligibility_status") for row in rows): return {"cycle_id": cycle_id, "diagnostics_available": False}
        prepared, counts = [], {}
        for row in rows:
            reasons = self._diagnostic_json(row["elimination_reasons"], []); reasons = reasons if isinstance(reasons, list) else []
            aggregate = self._diagnostic_json(row["aggregate_metrics"], {}); params = self._diagnostic_json(row["parameters"], {})
            prepared.append({**row, "reasons": reasons, "aggregate": aggregate if isinstance(aggregate, dict) else {}, "params": params if isinstance(params, dict) else {}})
            for code in reasons: counts[code] = counts.get(code, 0) + 1
        total, eligible_count = len(rows), sum(row["eligibility_status"] == "ELIGIBLE" for row in rows)
    ...
    def automatic_research_candidate_diagnostics(self, candidate_id: int) -> dict[str, Any] | None:
        with self.connect() as c:
            ...
            folds = [dict(item) for item in c.execute("SELECT * FROM strategy_discovery_folds WHERE candidate_id=? ORDER BY fold_number", (candidate_id,))]
        candidate = dict(row); aggregate = self._diagnostic_json(candidate.get("aggregate_metrics"), {}); reasons = self._diagnostic_json(candidate.get("elimination_reasons"), [])
        ...
        for code in reasons if isinstance(reasons, list) else []:
            ...
            if code in {"INSUFFICIENT_PROFITABLE_FOLDS", "INSUFFICIENT_BENCHMARK_BEATING_FOLDS"}:
                metric = "profitable_fold_count" if code == "INSUFFICIENT_PROFITABLE_FOLDS" else "benchmark_beating_fold_count"; observed = f"{aggregate.get(metric, '—')}/{aggregate.get('completed_fold_count', '—')}"; required = ">= 3/5"
    ...
    def persist_canonical_dataset_metadata(self, metadata:dict[str,Any])->None:
        """Migration-safe metadata upsert; deliberately never writes historical_candles."""
        now=utc_now(); key=str(metadata['metadata_key'])
        with self.connect() as c:
            c.execute("INSERT INTO canonical_dataset_metadata(metadata_key,schema_version,partition_fingerprint_version,dataset_fingerprint_version,dataset_fingerprint,payload,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(metadata_key) DO UPDATE SET schema_version=excluded.schema_version,partition_fingerprint_version=excluded.partition_fingerprint_version,dataset_fingerprint_version=excluded.dataset_fingerprint_version,dataset_fingerprint=excluded.dataset_fingerprint,payload=excluded.payload,updated_at=excluded.updated_at",(key,metadata['schema_version'],metadata['partition_fingerprint_version'],metadata['dataset_fingerprint_version'],metadata['dataset_fingerprint'],json.dumps(metadata,sort_keys=True),now,now))
    def canonical_dataset_metadata(self,key:str)->dict[str,Any]|None:
        with self.connect() as c:
            row=c.execute("SELECT payload FROM canonical_dataset_metadata WHERE metadata_key=?",(key,)).fetchone()
