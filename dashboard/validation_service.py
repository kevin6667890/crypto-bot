...
class ValidationService:
    ...
    def _job_gates(self, job_id: int, payload: dict[str, Any], checkpoint) -> dict[str, Any]:
        ...
        try:
            checkpoint(job_id,5,"validation.progress.loading_decisions",{}); decisions=self.repository.decisions(payload["filters"]); decisions=filter_decisions(decisions,payload["filters"]); checkpoint(job_id,45,"validation.progress.aggregating_decisions",{"total":len(decisions)}); result=aggregate_gate_funnel(decisions)
            near_count=0
            for index,decision in enumerate(decisions):
                ...
                if index and index%5000==0:checkpoint(job_id,45+int(index/max(1,len(decisions))*40),"validation.progress.persisting_near_misses",{"processed":index,"total":len(decisions),"saved":near_count})
    ...
    def _job_sensitivity(self,job_id:int,payload:dict[str,Any],checkpoint)->dict[str,Any]:
        ...
        try:
            ...
            for index,combo in enumerate(combos):
                checkpoint(job_id,5+int(index/max(1,len(combos))*90),"validation.progress.testing_combination",{"processed":index+1,"total":len(combos)});params=validate_parameters(combo);full=run_backtest(candles,payload["instrument"],payload["timeframe"],params,payload["start_ts"],payload["end_ts"],timeframe_datasets=mtf);oos=run_backtest(candles,payload["instrument"],payload["timeframe"],params,split+1,payload["end_ts"],timeframe_datasets=mtf);results.append(self._metric_row(combo,full,oos))
    ...
    def _job_robustness(self,job_id:int,payload:dict[str,Any],checkpoint)->dict[str,Any]:
        ...
        try:
            ...
            checkpoint(job_id,10,"validation.progress.trade_order_monte_carlo",{"simulations":payload["simulation_count"]});order=run_robustness(trades,initial,payload["simulation_count"],payload["random_seed"],"TRADE_ORDER",loss_threshold_pct=payload["loss_threshold_pct"],drawdown_threshold_pct=payload["drawdown_threshold_pct"]);checkpoint(job_id,55,"validation.progress.bootstrap",{"simulations":payload["simulation_count"]});bootstrap=run_robustness(trades,initial,payload["simulation_count"],payload["random_seed"],"BOOTSTRAP",loss_threshold_pct=payload["loss_threshold_pct"],drawdown_threshold_pct=payload["drawdown_threshold_pct"]);fee=stress_curve(trades,initial,[float(x) for x in payload["fee_multipliers"]],"fee");slippage=stress_curve(trades,initial,[float(x) for x in payload["slippage_multipliers"]],"slippage");missed=[]
