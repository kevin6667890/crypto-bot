...
const workspaceCopy = {
  zh: { eyebrow: "审计后市场研判", title: "AI 市场研判", audit: "AI 审计通过", auditTitle: "内容已通过引用、数值、方向、语义、关键位置与场景一致性检查", loading: "正在读取审计后的 AI 分析…", conclusion: "结论", observe: "观察", evidence: "关键依据", phase: "市场阶段", watching: "观察中", levels: "关键位置", noLevels: "当前无可靠关键位置", support: "支撑", resistance: "压力", level: "关键位", multi: "多周期", invalidation: "失效", scenario: "关注场景", updated: "报告生成于", dataTime: "事实截至", confidence: "置信边界", coverage: "数据覆盖", complete: "完整", partial: "部分", limited: "受限", risks: "风险 / 限制", report: "查看完整 AI 分析", staleTitle: "AI 分析已过期", staleBody: "历史内容不会作为当前市场结论展示，正在等待下一次有效分析。", lastValid: "最后有效分析", next: "下次评估", history: "查看历史完整 AI 分析", auditFailed: "最新 AI 分析未通过审计", unavailable: "暂无当前有效 AI 分析", unavailableBody: "等待下一次有效分析。未通过审计的报告正文不会展示。", dataQuality: "数据质量", dataLimit: "数据限制", current: "当前", aging: "即将过期", stale: "已过期" },
  en: { eyebrow: "Audited market intelligence", title: "AI Market View", audit: "AI audit passed", auditTitle: "Citations, values, direction, semantics, levels and scenarios passed consistency checks", loading: "Loading the audited AI analysis…", conclusion: "Conclusion", observe: "Observe", evidence: "Key evidence", phase: "Market phase", watching: "Monitoring", levels: "Key levels", noLevels: "No reliable key levels are available", support: "Support", resistance: "Resistance", level: "Key level", multi: "Multi-timeframe", invalidation: "Invalidation", scenario: "Scenario", updated: "Report generated", dataTime: "Facts as-of", confidence: "Confidence boundary", coverage: "Data coverage", complete: "Complete", partial: "Partial", limited: "Limited", risks: "Data limitations", report: "View full AI analysis", staleTitle: "AI analysis is stale", staleBody: "Historical content is not shown as a current market conclusion. Waiting for the next eligible analysis.", lastValid: "Last valid analysis", next: "Next evaluation", history: "View full historical AI analysis", auditFailed: "Latest AI analysis did not pass audit", unavailable: "No current eligible AI analysis", unavailableBody: "Waiting for the next eligible analysis. Audit-failed report content remains hidden.", dataQuality: "Data quality", dataLimit: "Data limitation", current: "Current", aging: "Aging", stale: "Stale" },
} as const;
...
export function WorkspaceAiBrief({ instrument }: { instrument: string }) {
  ...
  const noMaterialChange = brief?.scheduler?.last_evaluation_outcome === "SKIPPED_NO_MATERIAL_CHANGE";
  const displayAuditedReport = current || Boolean(stale && noMaterialChange);
  ...
  return <section className={`ai-insight-hero ${stale ? "stale" : ""}`} data-testid="workspace-ai6b-brief" aria-labelledby="ai-insight-title">
    <header className="ai-hero-header">
      ...
      <div className="ai-hero-badges"><span>{source(brief)}</span><span>{brief?.mode || "QUICK"}</span>{displayAuditedReport && brief?.audit.status === "PASSED" && <span className="audit-pass" title={copy.auditTitle}><CheckCircle2 size={14} /> {copy.audit} · {brief.audit.overall_score ?? 100} / 100</span>}{!latestFailed && fresh && <span className={`freshness-badge ${fresh.tone}`}><Clock3 size={13} />{fresh.label}</span>}</div>
    </header>
    <div className="ai-hero-content">{loading ? <div className="ai-hero-empty"><strong>{copy.loading}</strong></div> : displayAuditedReport && brief ? <div className="ai-hero-layout">
      <div className="ai-hero-primary">
        {noMaterialChange && <p className="ai-no-material-change" data-testid="ai-no-material-change">{language === "zh" ? "最近一次评估无重大事实变化，继续显示上一份审计报告。" : "No material facts changed at the latest evaluation; showing the previous audited report."}</p>}
      ...
      <div className="ai-hero-meta">
        <dl><div><dt>{copy.updated}</dt><dd>{when(brief.generated_at)}</dd></div><div><dt>{copy.dataTime}</dt><dd>{when(brief.market_snapshot_at)}</dd></div><div><dt>{copy.next}</dt><dd>{when(brief.scheduler?.next_tick)}</dd></div><div><dt>{copy.phase}</dt><dd>{enumText(brief.market_phase) || "—"}</dd></div><div><dt>{copy.confidence}</dt><dd>{enumText(brief.confidence) || "—"}</dd></div></dl>
