  ...
  return (
    <div className="workspace">
      <header className="workspace-topbar">
        <div className="workspace-brand"><TerminalSquare size={19} /><strong>Crypto-Bot</strong><span>Decision Workspace</span><div className="page-switch"><button className={activePage === "market" ? "active" : ""} onClick={() => setActivePage("market")}>Market Analysis</button><button className={activePage === "research" ? "active" : ""} onClick={() => setActivePage("research")}>Strategy Research</button><button className={activePage === "operations" ? "active" : ""} onClick={() => setActivePage("operations")}>Operations</button></div></div>
        <div className="market-controls">
          ...
          <select value={interval} onChange={(event) => setInterval(event.target.value)} aria-label="Chart interval"><option>1m</option><option>5m</option><option>15m</option><option>1h</option><option>4h</option><option>1D</option></select>
          <div className="language-switch" role="group" aria-label="Language"><button className={language === "en" ? "active" : ""} onClick={() => setLanguage("en")}>EN</button><button className={language === "zh" ? "active" : ""} onClick={() => setLanguage("zh")}>中文</button></div>
      ...
      {activePage === "market" ? <div className="workspace-grid">
        ...
        <main className="workspace-main">
          ...
          {paper?.flow && <section className="flow-panel"><div className="section-title"><div><span className="eyebrow">OKX public derivatives</span><h2>Order Flow & Open Interest</h2></div><small>{paper.flow.source}</small></div><div className="flow-grid"><article><div className="flow-head"><span>CVD · recent 100 trades</span><b className={paper.flow.cvd_delta >= 0 ? "positive" : "negative"}>{paper.flow.cvd_delta >= 0 ? "+" : ""}{paper.flow.cvd_delta.toLocaleString(undefined, { maximumFractionDigits: 0 })}</b></div><FlowChart points={paper.flow.cvd_series} zeroLine /><small>Signed spot taker notional; positive means buy-side dominance in this sample.</small></article><article><div className="flow-head"><span>SWAP Open Interest</span><b>${paper.flow.oi.toLocaleString(undefined, { maximumFractionDigits: 0 })}</b></div><FlowChart color="#0ea5e9" points={paper.flow.oi_history.map((point, index) => ({ time: Math.floor(new Date(point.created_at).getTime() / 1000) || index, value: point.oi }))} /><small>OKX {instrument.replace("-USDT", "-USDT-SWAP")} public OI · {paper.flow.oi_change_pct >= 0 ? "+" : ""}{paper.flow.oi_change_pct.toFixed(3)}% since last collection.</small></article></div></section>}
          {runtimeAnalysis?.contributions && <section className="explain-panel"><div className="section-title"><div><span className="eyebrow">Explainable rule engine</span><h2>Score Contribution</h2></div><b>{runtimeAnalysis.score}/100</b></div><div className="contribution-grid">{runtimeAnalysis.contributions.map((item) => <article key={item.key}><div><strong>{item.label}</strong><span className={item.status}>{item.max ? `${item.points}/${item.max}` : "Unavailable"}</span></div><div className="contribution-track"><i style={{ width: `${item.max ? item.points / item.max * 100 : 0}%` }} /></div><small>{item.detail}</small></article>)}</div></section>}
        ...
        <aside className="decision-panel">
          ...
          {runtimeAnalysis?.strategy_version && <div className="signal-lineage"><span>Version <b>{runtimeAnalysis.strategy_version}</b></span><span>Config <b title={runtimeAnalysis.config_hash}>{runtimeAnalysis.config_hash?.slice(0,10)}</b></span><span>Signal <b title={runtimeAnalysis.signal_id}>{runtimeAnalysis.signal_id?.slice(0,10)}</b></span><span>Collector Freshness <b>{runtimeAnalysis.updated_at ? new Date(runtimeAnalysis.updated_at).toLocaleTimeString() : "Starting"}</b></span></div>}
