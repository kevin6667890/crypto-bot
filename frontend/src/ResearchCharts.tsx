...
export function BacktestCandleChart({ candles, trades }: { candles: Array<{ ts: number; open: number; high: number; low: number; close: number }>; trades: BacktestTrade[] }) {
  ...
  if (!shown.length) return <div className="research-empty">Run a backtest to load confirmed OKX candles.</div>;
  ...
  return <div className="research-chart candle"><span>Confirmed OKX candles · last {shown.length} bars</span><svg viewBox="0 0 1000 250" preserveAspectRatio="none" role="img" aria-label="Candles and trade markers">
...
export function DistributionCharts({ trades, monthly }: { trades: BacktestTrade[]; monthly: Array<{ month: string; return: number }> }) {
  ...
  return <div className="distribution-grid">
    <div className="mini-chart"><strong>Trade R Distribution</strong><div className="bar-set">{counts.map((count, index) => <div key={buckets[index]}><i style={{ height: `${count / maxCount * 100}%` }} /><small>{buckets[index]}R+</small></div>)}</div></div>
    <div className="mini-chart"><strong>Monthly Returns</strong><div className="month-set">{monthly.slice(-18).map((item) => <div key={item.month} title={`${item.month}: ${item.return.toFixed(2)}%`}><i className={item.return >= 0 ? "up" : "down"} style={{ height: `${Math.abs(item.return) / maxMonth * 46}%` }} /><small>{item.month.slice(2)}</small></div>)}</div></div>
    <div className="mini-chart"><strong>Long / Short Results</strong>{side.map((item) => <div className="side-result" key={item.name}><b>{item.name}</b><span>{item.trades} trades</span><span>{item.trades ? (item.wins / item.trades * 100).toFixed(1) : "--"}% win</span><strong className={item.pnl >= 0 ? "positive" : "negative"}>{item.pnl.toFixed(2)}</strong></div>)}</div>
  </div>;
}
