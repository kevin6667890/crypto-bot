...
import { fetchPositionDetails, fetchPresentation, PresentationApiError, tripKillSwitch } from "./api";
...
export default function ShadowMarketAnalysisPage(){
  ...
  const [position,setPosition]=useState<Record<string,unknown>|null>(null);const sequence=useRef(new RequestSequence());const lastManual=useRef(0);const pollingStopped=useRef(false);
  ...
  const load=useCallback(async(reason:"cache-first"|"refresh"="cache-first")=>{
    if(!authorized||!token||pollingStopped.current)return;if(reason==="refresh"&&Date.now()-lastManual.current<1000)return;if(reason==="refresh")lastManual.current=Date.now();
    ...
    try{const next=await fetchPresentation({instrument,mode,language:reportLanguage,token,signal:request.signal});if(!sequence.current.accepts(request.sequence))return;if(next.instrument!==instrument||next.mode!==mode){void tripKillSwitch(token,"CONTEXT_MISMATCH","AUDITED_BODY_SELECTION_MISMATCH");pollingStopped.current=true;setAuthorized(false);setToken("");setValue(null);setError("AUDITED_BODY_SELECTION_MISMATCH_HARD_STOP");setPhase("error");return;}presentationCache.set(key,next);setValue(next);setPhase("idle");}
    catch(reasonValue){if((reasonValue as Error).name==="AbortError")return;if(!sequence.current.accepts(request.sequence))return;const code=reasonValue instanceof PresentationApiError?`${reasonValue.status}:${reasonValue.code}`:"CONTRACT_OR_NETWORK_ERROR";setError(code);setValue(null);setPhase("error");if(reasonValue instanceof PresentationApiError&&reasonValue.code==="PRESENTATION_CONTRACT_ERROR"){void tripKillSwitch(token,"UNAUDITED_BODY_DISPLAY","PRESENTATION_CONTRACT_ERROR");pollingStopped.current=true;setAuthorized(false);setToken("");}if(reasonValue instanceof PresentationApiError&&reasonValue.status===401){pollingStopped.current=true;setAuthorized(false);setToken("");}if(reasonValue instanceof PresentationApiError&&reasonValue.status===429)pollingStopped.current=true;}
  ...
  const authorize=(event:React.FormEvent)=>{event.preventDefault();presentationCache.clear();pollingStopped.current=false;setAuthScope(x=>x+1);setAuthorized(Boolean(token));};
  const loadPosition=async()=>{if(!value||pollingStopped.current)return;const controller=new AbortController();try{setPosition(await fetchPositionDetails({reportId:value.report_id,instrument,mode,token,signal:controller.signal}));}catch(reasonValue){if(reasonValue instanceof PresentationApiError&&reasonValue.status===401){pollingStopped.current=true;setAuthorized(false);setToken("");}if(reasonValue instanceof PresentationApiError&&reasonValue.status===429)pollingStopped.current=true;setError("POSITION_DETAILS_UNAVAILABLE");}};
