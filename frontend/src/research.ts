...
export type OptimizationFamily = { id:number; name:string; instrument:string; timeframe:string; start_ts:number; development_end_ts:number; holdout_start_ts:number; holdout_end_ts:number; final_oot_start_ts?:number; final_oot_end_ts?:number; family_fingerprint:string; holdout_revealed_at?:string; final_oot_revealed_at?:string; notes?:string; runs:Array<Pick<OptimizationRun,"id"|"status"|"seed"|"post_holdout_adjustment"|"search_space_changed"|"base_parameters_changed"|"holdout_revealed_at"> & {created_at:string; parent_run_id?:number; request?:OptimizationRun["request"]}> };
export type ValidationSuiteResult = {stage:string;instrument:string;timeframe:string;start_ts:number;end_ts:number;status:string;metrics?:BacktestMetrics;buy_hold_metrics?:{total_return?:number};data_quality?:Record<string,unknown>;error?:string};
export type ValidationSuite = {id:number; experiment_family_id:number; job_id?:number; status:string; source_optimization_run_id:number; source_trial_id:number; request?:Record<string,unknown>;policy_version?:string;results?:ValidationSuiteResult[];created_at?:string;completed_at?:string;error?:string;retry_of_suite_id?:number;attempt_number?:number};
...
export const researchApi = {
  ...
  walkForward: (payload: object) => request<ResearchJob>("/api/walk-forward", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }),
  ...
  optimizationFamilies: async () => (await request<{items:OptimizationFamily[]}>("/api/optimization/families")).items,
  optimizationFamily: (id:number) => request<OptimizationFamily>(`/api/optimization/families/${id}`),
  createOptimizationFamily: (payload:object) => request<OptimizationFamily>("/api/optimization/families", {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}),
