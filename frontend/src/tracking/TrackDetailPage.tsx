...
export default function TrackDetailPage({ trackId }: { trackId: string }) {
  ...
  return <main className="track-detail-page">
    ...
    <section className="track-evidence-columns">
      ...
      <article className="track-evidence-panel historical"><span className="product-eyebrow">{zh ? "历史证据 · 保存时基线" : "HISTORICAL EVIDENCE · SAVED BASELINE"}</span><h2>{baselineEventLabel(baseline.historical_summary || {}, zh)}</h2>{baselineSampleLabel(baseline.historical_summary || {}, zh) && <p>{baselineSampleLabel(baseline.historical_summary || {}, zh)}</p>}<small>{formatUtc(baseline.historical_tested_range.start, language)} → {formatUtc(baseline.historical_tested_range.end, language)}</small><p className="baseline-note">{zh ? "这份历史基线在保存后不会自动重算。" : "This historical baseline is not silently recalculated after saving."}</p></article>
    </section>
