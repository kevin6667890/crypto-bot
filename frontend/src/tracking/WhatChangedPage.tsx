...
import { formatCode, formatObserved, formatSemanticState, formatStatus, formatUtc } from "./state";
...
export default function WhatChangedPage() {
  ...
  return <main className="changes-page"><header className="tracking-hero"><span className="product-eyebrow">{zh ? "确定性的证据变化" : "Deterministic evidence changes"}</span><h1>{zh ? "发生了什么变化？" : "What changed?"}</h1><p>{zh ? "这里只展示已跟踪条件、质量或证据身份的实质变化。" : "Only material changes in tracked conditions, quality, or evidence identity appear here."}</p></header>
    ...
    <section className="changes-feed">{changes.map(({ track, evaluation }) => <a href={`/tracking/${encodeURIComponent(track.track_id)}`} key={evaluation.evaluation_id}><header><strong>{track.thesis_spec.instrument} · {track.thesis_spec.timeframe}</strong><time>{formatUtc(evaluation.evaluated_at, language)}</time></header>{evaluation.delta?.status_changed && <h2>{formatStatus(evaluation.delta.previous_status, language)} → {formatStatus(evaluation.overall_status, language)}</h2>}{evaluation.delta && (evaluation.delta.overall_change || evaluation.delta.leaf_changes?.length || evaluation.delta.group_changes?.length)
      ? <V2Delta delta={evaluation.delta} zh={zh} />
      : <div className="delta-list">{evaluation.delta?.condition_changes.map((item) => <p key={item.feature}><b>{formatCode(item.feature, language)}</b><span>{formatCode(item.from, language)} → {formatCode(item.to, language)}</span><small>{formatObserved(item.previous_observed_value, language)} → {formatObserved(item.current_observed_value, language)}</small></p>)}{evaluation.delta?.quality_changes.map((item) => <p key={`${item.feature}-quality`}><b>{formatCode(item.feature, language)}</b><span>{formatCode(item.from, language)} → {formatCode(item.to, language)}</span></p>)}</div>}<span className="change-link">{zh ? "查看跟踪项目 →" : "View thesis →"}</span></a>)}</section>
    <section className="market-changes-feed">{marketChanges.map((item, index) => <article key={`${item.current_as_of}-${index}`}><header><strong>{item.instrument} · {item.timeframe}</strong><time>{formatUtc(item.transition.transition_timestamp, language)}</time></header><span className="product-eyebrow">{zh ? "市场结构变化" : "MARKET STRUCTURE CHANGED"}</span><h2>{formatSemanticState(item.transition.from_state, language)} → {formatSemanticState(item.transition.to_state, language)}</h2><p>{item.transition.trigger_evidence.map((evidence) => formatCode(evidence, language)).join(" · ")}</p><small>{zh ? "现有 MarketState V2 的已确认语义变化" : "Confirmed semantic transition from the existing MarketState V2 engine"}</small></article>)}</section>
    </main>;
}
