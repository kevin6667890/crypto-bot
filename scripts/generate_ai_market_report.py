...
def main()->int:
 ...
 db=a.database or Path(tempfile.mkdtemp())/"ai_market_reports.db";migrate_database(db);repo=ReportRepository(db);model=a.model or ("fake-ai4" if a.provider=="fake" else "")
 ...
 if not a.show_prompt_hash:submitted.pop("prompt_hash",None)
