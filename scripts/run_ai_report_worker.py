...
def load_provider_secret()->None:
 path=os.getenv("AI_REPORT_API_KEY_FILE")
 if path:
  value=Path(path).read_text(encoding="utf-8").strip()
  if not value:raise RuntimeError("AI_REPORT_API_KEY_FILE is empty")
  os.environ["AI_REPORT_API_KEY"]=value
def main()->int:
 ...
 def factory(request):
  ...
  return DeepSeekAIReportProvider(request["model"])
