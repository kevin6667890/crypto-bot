...
from scripts.artifact_integrity import (
    MANIFEST_VERSION, ArtifactSeal, analyze_original_artifact,
    canonical_json_file, render_report,
)
...
ORIGINAL_EVIDENCE_COMMIT = "44e82342306f64a0ead5de121db158d5563e5246"
ORIGINAL_ARTIFACT_ID = "6f0adb0f61e63d3d26960d9b83f80e352e239af3c50bc7fcfa04dcbd2af7ed04"
ORIGINAL_ARTIFACT_REPO_PATH = (
    ".runtime/strategy-phase4a6d-bounded-identity-gate/" + ORIGINAL_ARTIFACT_ID)
ORIGINAL_ARTIFACT_DIR = Path(
    r"C:\Users\ASUS\PycharmProjects\crypto-bot-lifecycle-bounded-gate\.runtime"
) / "strategy-phase4a6d-bounded-identity-gate" / ORIGINAL_ARTIFACT_ID
...
def write_json(path: Path, value: Any) -> None:
    canonical_json_file(path, value)
...
def main() -> int:
    ...
    run_id = stable_hash({"version": VERSION, "integrity": MANIFEST_VERSION,
                          "mode": "RECOMPUTED_BOUNDED_GATE", "code_sha": code_sha,
    ...
    if any(output.iterdir()):
        raise RuntimeError("deterministic artifact directory already contains files")
    seal = ArtifactSeal(output)

    forensics = analyze_original_artifact(
        ROOT, ORIGINAL_ARTIFACT_DIR, ORIGINAL_EVIDENCE_COMMIT,
        ORIGINAL_ARTIFACT_REPO_PATH,
    )
    write_json(output / "original_artifact_forensics.json", forensics)
    write_json(output / "reseal_decision.json", {
        "version": "phase4a6d-reseal-decision-v1",
        "decision": "RECOMPUTED_BOUNDED_GATE",
        "reuse_allowed": False,
        "core_machine_filesystem_matches_original_manifest": forensics["core_json_filesystem_matches_manifest"],
        "core_machine_git_blobs_match_original_manifest": forensics["core_json_git_blobs_match_manifest"],
        "only_display_members_changed": forensics["only_display_members_changed"],
        "reason": "Core machine-readable Git blobs differ from the v1 manifest after Git clean-filter newline transformation; v1 is audit-only and cannot be re-signed.",
        "sha_values_directly_modified": False,
    })
    ...
    manifest = {
        ...
        "bounded_result_mode": "RECOMPUTED_BOUNDED_GATE",
        "integrity_manifest_version": MANIFEST_VERSION,
    ...
    machine_names = (
        "final_gate_decision", "gate_manifest", "lifecycle_transition_audit",
        "identity_continuity_audit", "checkpoint_resume_comparison", "direction_summary",
    )
    machine = {
        name: json.loads((output / f"{name}.json").read_bytes())
        for name in machine_names
    }
    if machine["final_gate_decision"]["passed"] != all(machine["final_gate_decision"]["conditions"].values()):
        raise RuntimeError("final decision cross-field validation failed")
    seal.write_bytes("report.md", render_report(machine))
    required = {
        "gate_manifest.json", "original_artifact_forensics.json", "reseal_decision.json",
        "feature_diff_audit.json", "dataset_integrity.json", "witness_results.json",
        "window_summary.json", "direction_summary.json", "parameter_summary.json",
        "propagation_audit.json", "identity_continuity_audit.json",
        "lifecycle_transition_audit.json", "checkpoint_schema_audit.json",
        "checkpoint_resume_comparison.json", "validation_access_audit.json",
        "oot_access_audit.json", "final_gate_decision.json", "report.md",
    }
    seal.prepare(required)
    seal.seal()
    seal.verify_unchanged()
    print(canonical_json({"artifact_path": str(output), "artifact_state": seal.state,
