...
def test_original_report_column_remains_pending_only():
    migration=Path("migrations/ai_report/001_ai_report_database.sql").read_text(encoding="utf-8")
    source=Path("dashboard/ai_market_analysis/report_repository.py").read_text(encoding="utf-8")
    assert "CHECK(audit_status='PENDING')" in migration and "UPDATE ai_market_reports" not in source
