...
def test_claim_identity_type_modality_timeframe_refs_stable():
    ...
    a=extract_claims("r",report);b=extract_claims("r",report);assert a==b and a[0]["claim_type"]=="TIMEFRAME_STRUCTURE" and a[0]["modality"]=="UNCERTAIN" and a[0]["timeframe_mentions"]==["周线"]
    assert a[0]["scope"]=="1W" and a[0]["role"]=="DETAIL" and a[0]["semantic_key"]=="TIMEFRAME_STRUCTURE:1W"
...
def test_macro_unavailable_status_is_a_limitation_not_a_macro_claim():
    ...
    assert claim["claim_type"]=="COVERAGE_METADATA"
    assert claim["semantic_key"]=="COVERAGE_METADATA:MACRO"
